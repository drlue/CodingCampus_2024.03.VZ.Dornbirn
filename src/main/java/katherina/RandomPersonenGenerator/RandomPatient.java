...
//    Zyndi
        ...
        private static final int[] malus = (createRandomMalus(random.nextInt(5, 40)));

        public static int[] createRandomAge(int size) {
            int[] result = new int[size];
            for (int index = 0; index < result.length; index++) {
                result[index] = random.nextInt(18, 100);
            }
            return result;
        }

        public static int[] createRandomMaxHealth(int size) {
            int[] result = new int[size];
            for (int index = 0; index < result.length; index++) {
                result[index] = random.nextInt(50, 150);
            }
            return result;
        }

        public static int[] createRandomMalus(int size) {
            int[] result = new int[size];
            for (int index = 0; index < result.length; index++) {
                result[index] = random.nextInt(5, 40);
            }
            return result;
        }


        public static String generatePatient() {
            return "Patient patient = new Patient(\"" + (vorname[random.nextInt(vorname.length)]) + "\"," +
                    "\"" + nachname[random.nextInt(nachname.length)] + "\","
                    + alter[random.nextInt(alter.length)] + "," +
                    "\"" + krankheit[random.nextInt(krankheit.length)] + "\","
                    + maxHP[random.nextInt(maxHP.length)] + "," +
                    malus[random.nextInt(malus.length)] + ");"
                    ;
        }

        public static int fetchHealth(katherina.week08.Schwarzwaldklinik.Patient patient) {
            int health = Arrays.stream(maxHP).sum() - Arrays.stream(malus).sum();
            return health;
        }
    }
